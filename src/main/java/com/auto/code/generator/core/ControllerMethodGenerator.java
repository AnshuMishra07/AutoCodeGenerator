	...
	public static void generatePostMethod(JCodeModel codeModel, JDefinedClass jc, ApiSpecification spec,
		...
		try {
			requestClass = Utility.getAppClasses(spec,
					basePackage + Constant.DOMAIN_PACKAGE_NAME + spec.getApiName() + "Request", autoGeneRatedFilePath);
			responseClass = Utility.getAppClasses(spec,
					basePackage + Constant.DOMAIN_PACKAGE_NAME + spec.getApiName() + "Response", autoGeneRatedFilePath);
		} catch (Exception e) {
			throw new Exception(
	...
	private static void extractQueryParam(JCodeModel codeModel, ApiSpecification spec, JMethod apiMethod,
		...
		if (spec.getQueryParams() != null && !spec.getQueryParams().isEmpty()) {
			for (Map.Entry<String, String> entry : spec.getQueryParams().entrySet()) {
				try {
					apiMethod
							.param(Utility.getAppClasses(spec, JavaType.getJavaType().get(entry.getValue()),
									autoGeneRatedFilePath), entry.getKey())
							.annotate(codeModel.ref(Constant.REQUEST_PARAM)).param("value", entry.getKey());
					parameterMap.put(entry.getKey(), Class.forName(JavaType.getJavaType().get(entry.getValue())));
